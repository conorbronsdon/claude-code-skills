...
3. **Date everything.** Every cited fact gets the date of the source page (or "undated"). Funding databases, team pages, and pricing pages go stale fast. A 14-month-old headcount number presented as current is a hallucination with a citation.
4. **Separate verified from claimed.** Anything sourced only from the company itself (deck, website, founder posts) is "claimed." Anything confirmed by an independent source (customer post, press with named sources, public repo, conference talk) is "verified." Label each traction item as one or the other.
...
6. **Confidentiality: never paste deck contents into web searches.** Deck notes the user provides may be under NDA or simply private. Use them to know what to look for, then search in generic terms ("[company] pricing", "[company] customers"). Never put deck numbers, customer names from the deck, or roadmap details into a search query or any external tool.

## Instructions

### Step 1: Intake

Collect what the user has. Required: company name. Optional: website, deck notes, founder names, round details (stage, raise amount, valuation, lead). Ask for the website if the name is ambiguous (many startups share names). Note today's date at the top of your scratch state; it anchors every recency judgment.
...
Every subagent prompt must include: the company name and website, today's date, the six evidence rules above verbatim, and a report cap (~600 words, citations included). If the user provided deck notes, pass only derived search targets ("verify whether they have enterprise customers"), never the deck text itself.
...
- Spot-check the 3-5 highest-stakes claims (prior exit, named customers, funding amounts) by fetching the cited URL yourself. If the page does not support the claim, demote it to "could not verify."
...
1. **Snapshot**: what they do in one plain sentence a non-specialist understands. Stage, round, ask (mark each as provided-by-user, verified, or could not verify).
...
3. **Market**: who actually pays (the buyer, not the user, when they differ). Bottom-up sizing with arithmetic shown and each input cited. Timing thesis: why this is buildable/sellable now and was not two years ago. No top-down TAM quotes unless cited, and even then label them vendor-reported.
...
9. **Verdict scaffold**: NOT a recommendation. Three parts: strongest signal for, strongest signal against, and what a fair valuation range depends on (the 2-3 unknowns that move it most). Close with: "Decision is yours. This memo is evidence, not advice."

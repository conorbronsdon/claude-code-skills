<!-- Ported from conorbronsdon/agent-skills eval-integrity for Hermes. -->

# Dimension briefs

One paste-ready audit brief per dimension for the `eval-integrity` skill. Each carries its
sub-checks, its search vocabulary, its own severity rule, and the report contract.

**The file paths in these briefs are illustrations, not expectations.** They come from one hardened
benchmark and show what good looks like. They are not paths the auditor should expect to find in
your repo, and their absence is not evidence of a gap — search by concept before rating anything
ABSENT.

**Search with `search_files`.** Where a brief gives a pattern, it is a starting vocabulary for that
concept, not the definition of it.
...
A warning on the grep patterns: they were derived from one benchmark (a single reference benchmark) and carry its vocabulary. The target repo may name the same concept something else entirely. A grep miss is weak evidence of ABSENT. Before rating any sub-check ABSENT, search by concept: read the scoring entry points, the CI workflows, and the docs for the practice under whatever names the target uses.
...
silently re-run, corrections are a new dated run. (reference implementation:
...
leaderboard. (reference implementation: scripts/generate_data.py assert_author_allowed,
...
holdout-leak reason; a CI step that exits non-zero if BENCH_HOLDOUT_DIR is set
because the artifact upload would publish transcripts; a holdout_set block with
sha256 + n_scenarios only. (reference implementation: .gitignore data/results/artifacts/,
...
length-bias OLS regression; atomic per-dimension rubrics. (reference implementation:
...
pass@k with the semantics documented. (reference implementation: docs/methodology.md statistics
...
(reference implementation: eval/resume.py, eval/cost.py, pyproject.toml, the reproducibility_note
...
gate reading models_failed to block a partial board. (reference implementation:
